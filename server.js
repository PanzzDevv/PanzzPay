...
function extractAmountFromText(input) {
  ...
  const text = typeof input === 'object'
    ...
    : String(input);
  // Match various Rp/IDR formats: "Rp1", "Rp10.000", "Rp 10.000,00", "Rp10.543", "+Rp10,000", "IDR 50000"
  const currencyMatch = text.match(/[+]?\s*(?:rp\.?\s*|idr\s*)([\d.,]+)/i);
  if (currencyMatch) {
    let str = currencyMatch[1].trim();
    // Strip trailing cents only if represented with a comma (e.g. ",00")
    if (/,(\d{2})$/.test(str)) {
      str = str.replace(/,\d{2}$/, '');
    }
  ...
  // Fallback: find any standalone number >= 1 near payment keywords
  const fallbackMatch = text.match(/(?:sebesar|nominal|total|bayar|terima)\s*:?\s*(\d[\d.,]*)/i);
  if (fallbackMatch) {
    let str = fallbackMatch[1].trim();
    if (/,(\d{2})$/.test(str)) {
      str = str.replace(/,\d{2}$/, '');
    }
    const amount = Number(str.replace(/[^\d]/g, ''));
